...
import { validateFolderNames, validateFileNames } from "src/utils/validation";
...
const createFiles = async (filePath: string, folderContents: FolderContentTemplate) => {
  try {
    validateFolderNames(folderContents.folder);
    validateFileNames(Object.keys(folderContents.name_and_content));
